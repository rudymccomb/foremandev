...
class hardened::services {
  ...
  service {
    [
      ...
      'mdmonitor',   #3.3.7
      'pcscd',    #3.3.10
      #'rhnsd',    #2.1.2.2 this comes on at boot @RS, removing this for now
      'snmpd',    #3.20.1
  ...
  package {
    ...
    #FOO specific
    'krb5-devel': ensure => absent;
    'krb5-server': ensure => absent;
    'krb5-appl-servers': ensure => absent;
  }
  ...
  #exec { 'check_presence':
  #command => '/bin/true',
  #onlyif => '/usr/bin/test -e /etc/X11/xinit/xserverrc',
  #notify => File['/etc/X11/xinit/xserverrc'],
  #}
